...
def _ensure_elevation(size: int, seed: int = 42) -> np.ndarray:
    ...
    elev = np.fromiter((fbm(vx[i], vy[i], vz[i]) for i in range(vx.size)), dtype=np.float32, count=vx.size)
...
def _colorize(elev: np.ndarray) -> np.ndarray:
    """Map elevation in [0,1] to RGB using a standard gradient."""
...
def generate_sphere_image(size: int = 512, radius: float = 0.9, rot=(0.0, 0.0, 0.0)) -> Image.Image:
    """Return a fully-lit sphere with cached terrain sampling. radius<1 zooms out."""
