...
| SESAM (CLIMBER-X) column energy/water closure | `enable_sesam_column_closure` | Keep experimental (SESAM adoption stage P4; **exit gate passed 2026-08-18**). Default-off pure diagnostic kernels in `sesam_thermo.py` implement the Appendix A3/A4 column-energy/water closure of Willeit et al. (2022): the (A40) prognostic `Ta` budget, the (A41)/(A43) `T2m`/`q2m` near-surface diagnostics, and the (A44)/(A45) precipitation formula (continuous relative-humidity efficiency on gross convergence-plus-evaporation, plus a land-only turnover term) that bypasses the supported path's row-target allocator entirely within this branch. Reuses `column_water.py`'s flux machinery for `Qq` and stage P3's AT/Aq diffusivities directly (confirmed from the source PDF to be the literal same symbols, not a new closure). Not wired into the supported climate path (zero default-path impact by construction). Full 512x1024, both-seasons exit-gate measurement: global precipitation DJF 2.70 / JJA 3.00 mm/day (target [0.5, 5], no target ever supplied), close to the incumbent row-target allocator's own DJF 2.80 / JJA 2.91 mm/day on the same state; water conservation relative residual 5.4e-7 (DJF) / 6.1e-6 (JJA); energy pure-transport relative residual 2.2e-16 (DJF) / 0.0 (JJA). Recorded in `docs/SESAM_GAP_ANALYSIS.md` section 7; promotion only through that staged plan (P5 radiation, then the P6 calibration window). |
| SESAM (CLIMBER-X) cloud scheme | `enable_sesam_radiation` | Keep experimental (SESAM adoption stage P5, sub-deliverable 1 of 5: clouds; in progress). Default-off pure diagnostic kernels in `sesam_radiation.py` implement the Appendix A6 cloud scheme of Willeit et al. (2022): the (A61)/(A62) humidity/vertical-velocity cloud fraction, (A63)/(A64) effective cloud-level vertical velocity, (A65)/(A66) inversion/low-cloud fraction with freeze-dry factor, (A67) cloud top height, and (A68) cloud optical thickness. Equations verified against the source PDF and, where the paper's notation was ambiguous, against the reference implementation's `src/atm/clouds.f90`/`time_step.f90` (read-only, per section 5) -- two findings recorded in the module docstring: (A65)'s un-numbered `r*` symbol is stage P4's already-built `surface_relative_humidity_star`, and (A65)'s "when r*>ra" is a continuous ramp in the reference implementation, not a hard branch (same pattern as (A44) at P4). Follows the published Table A5 constants only, not the live reference repo's since-retuned namelist defaults (e.g. its `c_cld_5=0.75` vs the paper's printed 0.5). Not wired into the supported climate path (zero default-path impact by construction). Recorded in `docs/SESAM_GAP_ANALYSIS.md` section 7; promotion only through that staged plan. |
| SESAM (CLIMBER-X) shortwave radiation | `enable_sesam_radiation` (same gate as the cloud scheme) | Keep experimental (SESAM adoption stage P5, sub-deliverable 2 of 5: shortwave). Default-off pure diagnostic kernels in `sesam_shortwave.py` implement the Appendix A7 two-band delta-Eddington shortwave scheme of Willeit et al. (2022): (A79)/(A80) scattering albedo, (A81)/(A82) cloud albedo, (A87)-(A89) water-vapour/aerosol transmission, (A94)-(A105) absorber-mass paths (water and aerosol share one formula), (A75)-(A78) planetary albedo and (A83)-(A86) surface transmission via one shared two-stream combinator, assembled end to end by `shortwave_radiation()`. Verified against the source PDF and the reference implementation (`src/atm/swr.f90`/`constants.f90`, read-only per section 5) -- two genuine **paper transcription errors found and corrected**: (A87)/(A88)'s visible/IR band labels are swapped (water vapour absorbs in near-IR, not visible/UV; the published PDF has them backwards), and (A97)'s cloud-thickness exponent has a sign error (printed unbounded `e^{+Dcld/Hq}`, corrected to the reference implementation's bounded `e^{-Dcld/Hq}`). Confirms ozone needs no climatology anywhere in the shortwave path -- (A90)/(A91) are fixed constants (0.96/1), not a field. Not wired into the supported climate path (zero default-path impact by construction). Recorded in `docs/SESAM_GAP_ANALYSIS.md` section 7; promotion only through that staged plan. |
| SESAM (CLIMBER-X) longwave radiation | `enable_sesam_radiation` (same gate as clouds/shortwave) | Keep experimental (SESAM adoption stage P5, sub-deliverable 3 of 5: longwave). Default-off pure diagnostic kernels in `sesam_longwave.py` implement the Appendix A8 15-level two-stream longwave scheme of Willeit et al. (2022): (A110)-(A112) water-vapour/CO2/ozone transmission, (A113) cloud transmission, (A108)/(A109) combination, (A114)-(A116) absorber mass paths, and the (A106)/(A107) flux-profile assembly (a Riemann-Stieltjes discretization guarded by two structural boundary-condition tests derived directly from the paper's own continuous equations, not just hand-value checks). The GMD paper states the absorber-mass integral only abstractly with no discretization; this module's closure (P1's exponential (A1) reference pressure profile, exact for well-mixed CO2) is independently verified against a second citable source found with the user's help: **Petoukhov, Ganopolski & Claussen (2003), PIK Report No. 81** -- SESAM's direct scientific ancestor -- whose section 6.1.2 confirms the same "quasi-exponential" closure in prose. This resolved a real mid-implementation scope question (the reference Fortran's method initially looked sourced only from an inaccessible report, closer to the "written from the paper" licensing line than prior P5 disambiguations); the PIK report closes that gap with a real citation instead. One item stays flagged (not blocking): CO2's ppm-to-column-mass conversion is a standard calculation but not verified against the paper's specific "cm" absorber-mass unit convention. Confirms ozone *does* need a real climatology here, unlike shortwave -- kept as an accepted external input, format still undecided. Not wired into the supported climate path (zero default-path impact by construction). Remaining P5 items: the (A10) tropopause radiative closure, the ozone-climatology decision, and the full TOA-flux exit-gate measurement. Recorded in `docs/SESAM_GAP_ANALYSIS.md` section 7; promotion only through that staged plan. |
