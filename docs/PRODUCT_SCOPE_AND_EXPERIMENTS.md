...
| Phase 3 boundary-layer land replacement | `enable_force_restore_boundary_layer`, `enable_force_restore_atmospheric_heat_convergence`, `enable_force_restore_conservative_land_air_exchange`, `enable_boundary_layer_horizontal_transport`, `enable_boundary_layer_capacity_aware_airsea_exchange`, `enable_boundary_layer_capacity_aware_free_air_transport`, `enable_boundary_layer_near_surface_cloud_temperature`, `enable_boundary_layer_split_invariant_cloud_memory`, `enable_boundary_layer_stability_dependent_exchange`, `enable_boundary_layer_interface_reservoir` | **Active Phase-3 construction; default-off, do not promote.** The 2026-08-15 compact matrix rejected every convergence-coupled force-restore candidate against the supported baseline, and the conservative full-column land-air exchange candidate failed its 32x64 directional screen. That result requires this distinct boundary-layer heat reservoir (defined mass/depth, conservative boundary-layer/free-air exchange). The convergence forcing is diagnosed from the supported advection/diffusion increment, converted to W m-2, and globally area-closed. No 128x256 promotion run is authorized; do not copy the legacy latitude trapezoids into this branch. |
| Pressure-defined grey radiation | `enable_pressure_defined_radiative_temperature_profile`, `enable_coupled_two_layer_grey_radiation` | **Retained default-off research code; redesign required.** The coupled two-layer grey budget is column-conservative (~1e-14 residual) and routes mid/upper gains through the closed three-level MSE column, but its 2026-08-15 32x64 screen was rejected: a +17.3 K air-temperature handoff shock from near-neutral bootstrapped mid/upper stability feeding the unfloored diabatic-interface omega path, plus an independent +11-13 W m-2 OLR-target calibration gap in the optical-depth solve. Next admissible work is a coupled-grey-aware spin-up/initialization or a structurally independent overturning bound, not a stability floor, omega cap, or damping term. See `docs/VERTICAL_THERMODYNAMIC_CLOSURE.md`. |
