...
| SESAM (CLIMBER-X) SLP reconstruction | `enable_sesam_dynamics` | Keep experimental (SESAM adoption stages P2-P3). Default-off pure diagnostic kernels implement the Appendix A2 sea-level-pressure construction of Willeit et al. (2022) in `sesam_dynamics.py` (A28)-(A39) and the 3-D wind assembly in `sesam_wind.py` (A16)-(A27), and the Appendix A5 eddy-kinetic-energy closure in `sesam_synoptic.py` (A50)-(A60): Eady production, drag dissipation, EKE-derived synoptic wind/vertical velocity, and wind stress. Not wired into the supported climate path (zero default-path impact by construction). The diagnostic exit-gate measurement against the prescribed-cell generator is recorded in `docs/SESAM_GAP_ANALYSIS.md` section 7; promotion only through that staged plan. |
